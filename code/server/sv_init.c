...
//  Cvar_Set( "nextmap", va("map %s", server) );
    ...
    if ( sv_pure->integer ) {
        ...
        SV_TouchFile( "vm/sof2mp_cgame.qvm" );
        SV_TouchFile( "vm/sof2mp_ui.qvm" );
    }
    else {
        Cvar_Set( "sv_paks", "" );
...
void SV_Init (void)
{
    ...
    sv_floodProtect = Cvar_Get ("sv_floodProtect", "1", CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_INIT );
    sv_maxIPConnections = Cvar_Get("sv_maxIPConnections", "10", CVAR_ARCHIVE);
    ...
    sv_master[0] = Cvar_Get("sv_master1", MASTER_SERVER, 0);
    sv_master[1] = Cvar_Get("sv_master2", LEGACY_MASTER_SERVER, 0);
