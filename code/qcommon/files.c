    ...
    else if(search->dir)
    {
        ...
        if(!unpure && fs_numServerPaks)
        {
            if(!FS_IsExt(filename, ".cfg", len) &&      // for config files
               ...
               !FS_IsExt(filename, ".mapcycle", len) && // for mapcycle files
...
long FS_FOpenFileRead(const char *filename, fileHandle_t *file, qboolean uniqueFILE)
{
    ...
    isLocalConfig = !strcmp(filename, "autoexec.cfg") || !strcmp(filename, Q3CONFIG_CFG) || !strcmp(filename, "s2pserver.cfg");
