...
===========================================================================
*/
// tr_image_png.c - The ioquake3 PNG decoder.
...
static struct BufferedFile *ReadBufferedFile(const char *name)
{
    ...
    BF = Z_Malloc(sizeof(struct BufferedFile));
    if(!BF)
    {
        return(NULL);
    }
    ...
    BF->Length = FS_ReadFile((char *) name, &buffer.v);
    BF->Buffer = buffer.b;
...
static uint32_t DecompressIDATs(struct BufferedFile *BF, uint8_t **Buffer)
{
    ...
    *Buffer = DecompressedData;
    ...
    CompressedData = Z_Malloc(CompressedDataLength);
    if(!CompressedData)
    {
        return(-1);
    }
    ...
    DecompressedData = Z_Malloc(puffDestLen);
    if(!DecompressedData)
    {
        Z_Free(CompressedData);
...
void R_LoadPNG(const char *name, byte **pic, int *width, int *height, qboolean rawIndexed)
{
    ...
    *pic = NULL;
    ...
    || IHDR_Width > INT_MAX / Q3IMAGE_BYTESPERPIXEL / IHDR_Height)
    {
        ...
        Com_Printf(S_COLOR_YELLOW "R_LoadPNG: WARNING: %s: invalid image size\n", name );
